...
async def add_resource(request: web.Request) -> web.Response:
    ...
    main.LOGGER.info("Added resource %s -> %s (%s, group=%s)", resource["name"], resource["url"], resource.get("resource_type", "http"), resource.get("group_name", "auto"))
...
async def _probe_resource(resource: dict) -> tuple[str, dict]:
    ...
    try:
        ...
        try:
            return resource_id, {"online": True, "status": upstream.status, "latency_ms": round((time.monotonic() - started) * 1000)}
        finally:
            upstream.release()
    except (ClientError, asyncio.TimeoutError, OSError):
        return resource_id, {"online": False, "status": None, "latency_ms": round((time.monotonic() - started) * 1000)}
